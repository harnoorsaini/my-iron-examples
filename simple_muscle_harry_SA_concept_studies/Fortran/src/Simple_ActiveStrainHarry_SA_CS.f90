...
!
  ...
  ! spatial activation function parameters
  ! gamma = value at beta (must be non-zero!) 
  ! beta = max distance of incluence in mm
  REAL(CMISSRP), PARAMETER :: gamma=0.05_CMISSRP, beta=10.0_CMISSRP 
    
  ! Test program parameters
  REAL(CMISSDP), PARAMETER :: PI=4.0_CMISSDP*DATAN(1.0_CMISSDP)
  
  ! rectangular TA
  REAL(CMISSRP), PARAMETER :: WIDTH=50.0_CMISSRP !x
  REAL(CMISSRP), PARAMETER :: LENGTH=15.0_CMISSRP !y
  REAL(CMISSRP), PARAMETER :: HEIGHT=10.0_CMISSRP !z
  INTEGER(CMISSIntg), PARAMETER :: NumberGlobalXElements=10
  INTEGER(CMISSIntg), PARAMETER :: NumberGlobalYElements=2
  INTEGER(CMISSIntg), PARAMETER :: NumberGlobalZElements=3

  ! Timesteps and activation sources
  INTEGER(CMISSIntg), PARAMETER :: TIMESTEPS=50
  INTEGER(CMISSIntg), PARAMETER :: TotalNumberOfSources=6
...
!
  ...
  !           ii) The saturation activation is 1 
      ...
      ENDDO 
  !   
  ! -------------------------CHANGES TO BE IMPLEMENTED----------------------------------- 23.05.17
  ! - The spatial activation function alpha_spatial_wt needs to be normalized to the sum of the spatial function
  ! - The activation sources need to be offset to account for the fat/skin layer 
  !     d_hat_ij(t_n) = d_ij(t_n) + omega
  !   - This can actually be done in the model itself by converting the layer immediately under the AS nodes to passive mat.-type 
  ! - The number of activated fibres need by a given AS needs to be known; i.e. if alpha_spatial_wt > gamma; add the fibre to
  !     N_MF_j
  ! - Once all of this is implemented; the function to compute the effective spatial excitation 
  !     f_hat(d_ij(t_n)) = f(d_hat_ij(t_n))/(sum(f(d_hat_ij(t_n)))*N_MF_j)
        ...
        CASE(3) ! Node based
          ...
          DO node_idx=1,TotalNumberOfNodes 
            ...
            IF(NodeDomain==ComputationalNodeNumber) THEN                
              ...
              !WRITE(*,*) "a_t, a_s_sum, a_s_t: ", alpha_t, alpha_spatial_wt_sum, alpha_spatial_temporal
