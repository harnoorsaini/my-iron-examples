...
for $i (1..10)
  {
	for $j (0..3)
		{
	 		$filename = sprintf("ActiveStrain_TransIso_%d.part%d.exnode", $i, $j);
...
for $j (0..3)
  { 
	$filename = sprintf("ActiveStrain_TransIso.part%d.exelem", $j);
	gfx read elements "$filename";
  }
...
gfx modify g_element "Region" element_points coordinate deformed_geom glyph arrow_line general size 0.1 scale_factors 0.1 orientation Fibre material red	
