...
!    & CMFE_PROBLEM_NO_SUBTYPE,Err)
  ...
  DO node_idx=1,SIZE(RightSurfaceNodes,1)
    ...
    IF(NodeDomain==ComputationalNodeNumber) THEN
      CALL cmfe_BoundaryConditions_SetNode(BoundaryConditions,DependentField,CMFE_FIELD_U_VARIABLE_TYPE,1,1,NodeNumber,1, &
        & CMFE_BOUNDARY_CONDITION_FIXED_INCREMENTED,1.0_CMISSRP*WIDTH,Err)
    ENDIF
  ENDDO
 ...
 ! CALL cmfe_Problem_Solve(Problem,Err)
  ...
  DO i=1,TIMESTEPS
    ...
    ! DUMMY SOLVE  -  "fill in" any non-initalized values
 ...
 ! ===== ////// ====
 ! ===== UPDATE ALL MATERIAL POINTS  ====
    VALUE = 0.0_CMISSRP
    CALL cmfe_Field_ComponentValuesInitialise(MaterialField,CMFE_FIELD_U_VARIABLE_TYPE,CMFE_FIELD_VALUES_SET_TYPE,5, &
      & VALUE,Err)

 ! ===== ELMENET BASED UPDATE ====
    !   Loop over all elements  and set the activation individually at each time step
 !   DO elem_idx=1,NumberOfElementsFE
!
 !     IF (elem_idx .GE. 5) THEN
  !    VALUE= 0.0_CMISSRP !X(i)
   !   ELSE
    !  VALUE = 0.0_CMISSRP
     ! END IF

      !VALUE = 0.5_CMISSRP ! X(i)
      
      !CALL cmfe_Field_ParameterSetUpdateElement(MaterialField,CMFE_FIELD_U_VARIABLE_TYPE,CMFE_FIELD_VALUES_SET_TYPE,&
      !& elem_idx,5,VALUE,Err)
    !END DO
   
  ! ===== NODE BASED UPDATE ====

!cmfe_Field_ParameterSetUpdateNodeDPNumber(regionUserNumber,fieldUserNumber,variableType,fieldSetType, &
 !   & versionNumber,derivativeNumber,userNodeNumber,componentNumber,value,err)

    !DO node_idx=1,125 !NumberOfElementsFE
    !  IF ((NODE_IDX .LE. 10) .OR. (node_idx .GE. 100)) THEN
    !    VALUE= 0.5_CMISSRP
     ! ELSE
     !   VALUE=0.5_CMISSRP*0.999999_CMISSRP
     ! ENDIF
     ! WRITE(*,*) VALUE
     ! CALL cmfe_Field_ParameterSetUpdateNode(MaterialField,& 
    !  & CMFE_FIELD_U_VARIABLE_TYPE,CMFE_FIELD_VALUES_SET_TYPE,1, 1, node_idx,5,VALUE,Err)      
    !END DO

 ! ===== GAUSS POINT BASED UPDATE ====
!    DO elem_idx=1,NumberOfElementsFE

 !     VALUE=X(i)

      !IF ( i.GE.10 ) THEN
      !  VALUE=X(i)*elem_idx/NumberOfElementsFE
      !ENDIF

      !WRITE(*,*) X(i)*elem_idx/NumberOfElementsFE, X(i)
   ...
   !     & gauss_idx, elem_idx, 5, VALUE, Err)
     ! END DO 
!cmfe_Field_ParameterSetUpdateGaussPointDPNumber(regionUserNumber,fieldUserNumber,variableType,fieldSetType, &
 ...
 ! ===== ////// ====
  ...
  ENDDO
   ...
   BCLOAD = BCLOAD + 0.001_CMISSRP
    ...
    IF (i.LE.9) THEN
      WRITE(filename, "(A23,I1)") "ActiveStrain_Isometric_", i
      filename=trim(filename)
    ELSEIF (i.LE.99) THEN
      WRITE(filename, "(A23,I2)") "ActiveStrain_Isometric_", i
      filename=trim(filename)
    ELSE  
      WRITE(filename, "(A23,I3)") "ActiveStrain_Isometric_", i
      filename=trim(filename)
    ENDIF
    
!BCLOAD = BCLOAD + 0.01_CMISSRP
